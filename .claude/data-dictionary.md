# Data Dictionary — Study CSV

The timed study app writes raw columns through `DataExporter`; cleaning columns
are appended by `scripts/clean_keystrokes.py`. Rows are never deleted, only
flagged.
...
| `text_after` | str | Field text after this event |
| `edit_source` | str | `key`, `candidate`, `autocorrection`, or another edit origin |
| `edit_kind` | str | Original keyboard edit lineage: `insert`, `delete`, or `replace` |
| `original_text` | str | Text replaced/deleted by the originating keyboard action |
| `emitted_text` | str | Complete text emitted by the originating keyboard action |
| `suggestions_offered` | str | Pipe-separated suggestion-bar texts visible at edit time |
| `selected_suggestion` | str | Suggestion/autocorrect text chosen for this edit, if any |
| `touch_gesture_json` | JSON str | Full began/moved/ended samples, absolute/precise/local/normalized coordinates, radius, radius tolerance, force, maximum force, touch type, hit targets, slide/cancel state, and gesture duration. Candidate-bar taps are identified by their target. |
